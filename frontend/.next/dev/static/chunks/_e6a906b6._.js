...
const REAL_MISSION_LOGBOOK = [
    ...
    // NIVEAU 4 : Le Commandant est malade
    {
        id: 4,
        description: "Le Commandant ne doit gagner AUCUN pli.",
        constraints: [
            {
                type: 'NO_TRICKS',
                args: {
    ...
    // NIVEAU 6 : Ordre Strict
    {
        id: 6,
        description: "3 Tâches à faire dans l'ordre (1 > 2 > 3).",
        constraints: [
            {
                type: 'TARGET_CARD',
                args: {
    ...
    {
        id: 18,
        description: "Le joueur à droite du Commandant (Est) doit gagner exactement 2 plis.",
        constraints: [
            {
                type: 'TRICK_COUNT',
                args: {
...
const useGameLogic = ()=>{
        ...
        try {
            const res = await fetch('http://localhost:8000/solve-game', {
                ...
                body: JSON.stringify({
                    ...
                    constraints: globalConstraints // <--- AJOUT ICI
                }),
    ...
    const autoFindSolution = async ()=>{
        ...
        let attempts = 0;
        let found = false;
        const missionDef = MISSION_LOGBOOK.find((m)=>m.id === bookMissionId);
    ...
    const loadMissionFromBook = (id)=>{
        setBookMissionId(id);
        const missionDef = __TURBOPACK__imported__module__$5b$project$5d2f$data$2f$real_missions$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["REAL_MISSION_LOGBOOK"].find((m)=>m.id === id);
        if (!missionDef || allCards.length === 0) return;
        // 1. Identification des rôles
        const commanderCard = allCards.find((c)=>c.color === 'Rocket' && c.value === 4);
        const commanderIdx = commanderCard ? commanderCard.owner : 0; // Par défaut 0 si pas trouvé
        ...
        // 2. Préparation du deck de tâches (sans fusées)
        const validTaskCards = allCards.filter((c)=>c.color !== 'Rocket');
        const shuffledTasks = [
            ...validTaskCards
        ].sort(()=>Math.random() - 0.5);
        let taskIndex = 0;
        // 3. Traitement des contraintes
        missionDef.constraints.forEach((c)=>{
            // CAS A : C'est une tâche carte classique
            if (c.type === 'TARGET_CARD') {
                const count = c.args.count || 1;
                const tokens = c.args.tokens || Array(count).fill('None');
                for(let i = 0; i < count; i++){
                    if (taskIndex >= shuffledTasks.length) break;
                    // Distribution par défaut : Commandant puis sens horaire
                    const assignedOwner = (commanderIdx + taskIndex) % 4;
                    newMissions.push({
                        cardColor: shuffledTasks[taskIndex].color,
                        cardValue: shuffledTasks[taskIndex].value,
                        ownerIndex: assignedOwner,
                        token: tokens[i] || 'None'
                    });
                    taskIndex++;
                }
            } else if (c.type === 'NO_TRICKS' || c.type === 'TRICK_COUNT') {
                const targetPlayer = resolvePlayer(c.args.who);
                newConstraints.push({
                    type: c.type,
                    player: targetPlayer,
                    count: c.args.count
                });
                addLog(`ℹ️ Contrainte ajoutée : Joueur ${[
                    'Sud',
                    'Ouest',
                    'Nord',
                    'Est'
        ...
        setGlobalConstraints(newConstraints); // <--- On stocke ça
...
//# sourceMappingURL=_e6a906b6._.js.map